...
| **Use of DataHub** | Lineage + observed queries + glossary + contract writeback, via the first-party [Agent Context Kit](https://docs.datahub.com/docs/dev-guides/agent-context/agent-context) (22 tools; we run it with **mutations off** and keep writes in the deterministic path) and the MCP server |
| **Beyond what DataHub ships** | The kickoff session's own framing — *the point isn't wiring an LLM to DataHub over MCP; it's solving a problem your stack keeps having.* Search, lineage reads, text-to-SQL, quality monitoring and metadata enrichment are already shipped (MCP Server, Skills, Analytics Agent), so none of those is the contribution here. What isn't shipped: **three detectors** DataHub has no equivalent for (dbt manifest diff, `sqlglot` expression diff, glossary comparison), **cross-referencing** a proposed change against live reality to reach a *verdict* rather than a summary, and **two writebacks** — a PROPOSED Data Contract plus mergeable compatibility code. The catalog is the evidence base, not the feature |
