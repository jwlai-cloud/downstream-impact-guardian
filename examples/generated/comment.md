...
**on `fct_orders`** — finance-scheduled-queries@dig-demo-sandbox.iam (bigquery scheduled query):
```sql
SELECT order_date, SUM(order_total) AS revenue
FROM `dig-demo-sandbox.fiction_retail.fct_orders`
...
### Writeback 1 — Data Contract in DataHub

ℹ️ No DataHub credentials in this run; the exact contract payload the agent would submit is recorded below.

<details><summary>Contract payload the agent submits</summary>

```json
{
  "entityUrn": "urn:li:dataset:(urn:li:dataPlatform:bigquery,dig-demo-sandbox.fiction_retail.fct_orders,PROD)",
  "schema": [
    {
  ...
  "properties": [
    {
      "key": "proposedBy",
      "value": "downstream-impact-guardian"
    },
    {
      "key": "status",
      "value": "PROPOSED"
